...
Module.BringEnemy = function()
    ...
    maxPull = maxPull or 3
    pcall(sethiddenproperty, plr, "SimulationRadius", math.huge)
    local list = {}
    for _,v in ipairs(workspace.Enemies:GetChildren()) do
        if v.Name ~= PosName or v.Name == "Blank Buddy" then continue end
        ...
        if hum and pp and hum.Health > 0 then
            ...
            if dist <= Settings.BringDistance then
                list[#list+1] = {m=v, d=dist}
            end
        end
    end
    table.sort(list, function(a,b) return a.d < b.d end)
    for i = 1, math.min(maxPull, #list) do
        local v = list[i].m
...
function Module.GetNearest()
    for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
	    if (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 2000 and Module.IsAlive(v) then
	        return v
	    end
	end
    return nil
